...
# --- VORTEX v1.8.19e MULTI PAPER FUTURES ---
try:
    _pf_open=PaperFutures.open_position
    _pf_check=PaperFutures.check_stops
    _pf_close=PaperFutures.close_position

    def _pf_ensure_multi(self):
        if not hasattr(self,'positions') or not isinstance(getattr(self,'positions',None),dict):
            self.positions={}
        try:
            if getattr(self,'pos',None) is not None:
                sym=str(getattr(self.pos,'symbol','') or '').upper()
                if sym: self.positions[sym]=self.pos
                self.pos=None
        except Exception:
            pass

    def _pf_all(self):
        _pf_ensure_multi(self); return dict(self.positions)

    def _pf_get(self,symbol=None):
        _pf_ensure_multi(self)
        if symbol: return self.positions.get(str(symbol or '').upper())
        return next(iter(self.positions.values())) if self.positions else None

    def _pf_open_multi(self,*args,**kwargs):
        _pf_ensure_multi(self)
        symbol=str(kwargs.get('symbol') or (args[0] if args else '') or '').upper()
        max_open=int(getattr(getattr(CONFIG,'risk',None),'max_open_futures_positions',1))
        if symbol and symbol in self.positions: return {'code':'ERROR','msg':'Position already open for symbol'}
        if len(self.positions)>=max_open: return {'code':'ERROR','msg':'Max paper futures positions reached'}
        self.pos=None
        res=_pf_open(self,*args,**kwargs)
        if isinstance(res,dict) and res.get('code')=='00000' and self.pos is not None:
            sym=str(getattr(self.pos,'symbol','') or symbol).upper()
            self.positions[sym]=self.pos
            self.pos=None
        return res

    def _pf_check_symbol(self,symbol,current_price):
        _pf_ensure_multi(self)
        sym=str(symbol or '').upper(); pos=self.positions.get(sym)
        if pos is None: return None
        self.pos=pos
        res=_pf_check(self,current_price)
        if self.pos is None: self.positions.pop(sym,None)
        else:
            self.positions[sym]=self.pos
            self.pos=None
        return res

    def _pf_check_multi(self,current_price):
        _pf_ensure_multi(self)
        first=_pf_get(self)
        if first is None: return None
        return _pf_check_symbol(self,getattr(first,'symbol',''),current_price)

    def _pf_close_multi(self,price,reason='MANUAL',symbol=None):
        _pf_ensure_multi(self)
        sym=str(symbol or '').upper()
        if not sym:
            first=_pf_get(self); sym=str(getattr(first,'symbol','') or '').upper() if first else ''
        pos=self.positions.get(sym)
        if pos is None: return None
        self.pos=pos
        res=_pf_close(self,price,reason)
        self.positions.pop(sym,None)
        self.pos=None
        return res

    PaperFutures._vortex_ensure_multi=_pf_ensure_multi
    PaperFutures.get_all_positions=_pf_all
    PaperFutures.get_position=_pf_get
    PaperFutures.open_position=_pf_open_multi
    PaperFutures.check_position=_pf_check_symbol
    PaperFutures.check_stops=_pf_check_multi
    PaperFutures.close_position=_pf_close_multi
except Exception:
    pass
# --- END VORTEX v1.8.19e MULTI PAPER FUTURES ---
